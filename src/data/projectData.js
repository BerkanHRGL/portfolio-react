...
<div class="final-presentation-image">
  ...
  portfolio: {
    ...
    content: `
    <h2>PROJECT OVERVIEW</h2>
    <p>This portfolio website is a personal digital showcase built to present my work as a media design student. It started as a vanilla HTML, CSS and JavaScript project and was later fully rebuilt using React and Vite — keeping the same retro OS aesthetic while gaining a proper component-based architecture, cleaner state management, and a much smoother development experience.</p>
    <div class="project-info-row">
      <div class="tools-section">
        <div class="detail-section-label">TOOLS USED</div>
        <div class="detail-tags">
          ...
          <span class="detail-tag">Vite</span>
          <span class="detail-tag">HTML</span>
          <span class="detail-tag">CSS</span>
          <span class="detail-tag">JavaScript</span>
        </div>
...
<div class="presentation-section">
  <h2>DEVELOPMENT</h2>
  <p>The portfolio started as a vanilla HTML, CSS and JavaScript project where I built everything from scratch — draggable windows, a boot sequence, a login screen with a typing animation, and a project showcase with a custom canvas glitch effect. Getting all those interactions working without any framework taught me a lot about how the browser actually works.</p>

  <p>After building the initial version I decided to rebuild the entire portfolio in React with Vite. The goal was to keep the exact same retro OS look and feel while learning how a real frontend framework works in practice. I restructured everything into reusable components — a generic Window component with a custom useDraggable hook, separate windows for About, Projects, Contact and Learning Outcomes, and a full showcase and detail view for projects.</p>

  <p>State management is handled with React's useState and useEffect hooks. The app flows through three screens — boot, login and desktop — managed by a single state variable in App.jsx. Windows track their open/active state in Desktop.jsx using arrays and callbacks passed down as props.</p>

  <p>One of the more interesting challenges was making the HTML project content (stored as template literals) interactive inside React. Since it's rendered with dangerouslySetInnerHTML, I used event delegation on the content containers to handle accordion toggles, PDF links and external URLs without attaching individual listeners to every element.</p>

  <p>The cyberpunk glitch transition on the project showcase is a canvas-based animation using requestAnimationFrame, with bent bars drawn using quadratic Bezier curves for a more organic look. It was one of my favorite parts to build.</p>

  <h3>VERSION CONTROL</h3>
  <div class="version-control-container">
    <div class="version-control-text">
      <p>I used GitHub Desktop throughout the project to track changes across both the vanilla JS version and the React rebuild.</p>

      <p>Keeping a clean commit history helped a lot when switching between the two versions and making sure nothing got lost during the migration.</p>
